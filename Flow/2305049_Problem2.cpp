...
void solve(){
    ll n,k,m; cin>>n>>k>>m;
    vector<vector<pair<pll,pll>>> adj(n+2);
    vpll input(m);
    for(int i=0;i<m;i++){
        ll x,y; cin>>x>>y;
        adj[x].pb({{y,1},{1,adj[y].size()}});
    ...
    for(int i=0;i<k;i++){
        adj[s].pb({{i,1},{1,adj[i].size()}});
        adj[i].pb({{s,2},{0,adj[s].size()-1}});
    }

    for(int i=k;i<n;i++){
        adj[i].pb({{t,1},{1,adj[t].size()}});
        adj[t].pb({{i,2},{0,adj[i].size()-1}});
    }
